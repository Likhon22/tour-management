    ...
    return (
        <div className="py-8 px-6 lg:px-10 max-w-7xl mx-auto pb-32">
            <header className="mb-8 flex justify-between items-center">
                <div>
                    <h1 className="text-4xl font-black tracking-tight mb-1 font-inter text-slate-900 dark:text-white">Trip <span className="text-sky-500">Summary</span></h1>
            ...
            <section className="space-y-6">
                ...
                >
                    ...
                    <p className="text-sky-100/60 dark:text-sky-200/60 text-[10px] font-black uppercase tracking-[0.2em] mb-1">Cash In Hand</p>
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-6 border-t border-white/10 uppercase font-black text-[10px] tracking-widest">
                        <div>
                            <p className="opacity-50 mb-1">Total Collected</p>
                            <p className="text-emerald-400 font-bold text-sm"><Taka />{stats.totalCollected}</p>
                        </div>
                        <div className="text-right">
                            <p className="opacity-50 mb-1">Total Spent</p>
                            <p className="text-rose-400 font-bold text-sm"><Taka />{stats.totalSpent}</p>
                        </div>
                    </div>
                </motion.div>

                {/* Per Person Share */}
                <div className="bg-white dark:bg-sky-500/5 p-6 rounded-[2rem] border border-slate-200 dark:border-sky-500/10 flex items-center justify-between shadow-xl shadow-slate-200/40 dark:shadow-none">
                    <div>
                        <p className="text-[10px] font-black text-slate-400 dark:text-sky-400 uppercase tracking-widest mb-1">Average Cost Per Person</p>
                        <p className="text-2xl font-black text-slate-900 dark:text-sky-100 tracking-tight"><Taka />{Math.round(stats.individualShare)}</p>
                    </div>
                    <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl shadow-sm">
                ...
                <div>
                    <div className="flex items-center gap-2 mb-4 px-2">
                        ...
                        <h3 className="font-black text-lg uppercase tracking-tight text-slate-900 dark:text-white">Settlement Status</h3>
                    </div>
                    <div className="space-y-4">
                        {participants.map((p: any) => {
                            const contribution = Number(stats.contributionsRecord[p._id] || 0);
                            return (
                                <div key={p._id} className="bg-white dark:bg-slate-800/40 p-5 rounded-[2.2rem] border border-slate-200 dark:border-slate-800/50 flex justify-between items-center group shadow-xl shadow-slate-200/40 dark:shadow-none transition-all hover:border-sky-200">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-2xl bg-sky-50 dark:bg-sky-500/20 flex items-center justify-center font-black text-sky-600 dark:text-sky-400 text-xl uppercase shadow-sm">
                                            {p.name ? p.name.charAt(0) : "?"}
                                        </div>
                                        <div>
                                            <span className="font-black text-slate-900 dark:text-white text-lg block leading-none mb-1 tracking-tight">{p.name}</span>
                                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Navigator</span>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <span className="text-[10px] font-black tracking-widest uppercase text-emerald-600 dark:text-emerald-500 mb-0.5 block">
                                            Contributed
                                        </span>
                                        <p className="font-black text-xl tracking-tighter text-emerald-600 dark:text-emerald-400">
                                            <Taka />{contribution}
                                        </p>
                                    </div>
                                </div>
                ...
                <div>
                    <div className="flex items-center gap-2 mb-4 px-2">
                        ...
                        <h3 className="font-black text-lg uppercase tracking-tight text-slate-900 dark:text-white">Cost Breakdown</h3>
                    </div>
                    <div className="grid grid-cols-1 gap-4">
                        {Object.entries(stats.categoryCosts || {}).map(([name, amount]: [string, any]) => (
                            <div key={name} className="bg-white dark:bg-slate-800/40 p-6 rounded-[2.5rem] border border-white dark:border-slate-800/50 shadow-xl shadow-slate-200/40 dark:shadow-none transition-all hover:border-orange-100">
                                <div className="flex justify-between items-center mb-4">
                                    <div className="flex items-center gap-2">
                                        <span className="text-xl">{getEmojiForCategory(name)}</span>
                                        <span className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.15em] leading-tight">{name}</span>
                                    </div>
                                    <span className="font-black text-xl tracking-tight text-slate-900 dark:text-white"><Taka />{amount}</span>
                                </div>
